  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const processedLeaveRequests = (leaveData.data || []).map((request: any) => {
        ...
        // Build full name from joined employee data
        let fullName = request.full_name || 'Unknown Employee'
        if (employee) {
        ...
        // Get leave type key
        let leaveTypeKey = request.leave_type || 'unknown'
        if (leaveType) {
          leaveTypeKey = leaveType.key || leaveType.leave_type || request.leave_type || 'unknown'
        }
      ...
      // Fetch unread verification request notifications for this super admin
