...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await employeeService.verifyId(employeeId)

    return NextResponse.json({ success: true, message: "Employee ID verified" })
  } catch (error) {
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to verify employee ID" },
      { status: 500 },
    )
  }
}
